...
### 1. Dosya Hazırlığı
Bu repodaki dosyaları indirin ve klasör adının `qnbefatura` olduğundan emin olun. Klasörü şu dizine yükleyin:
`/modules/addons/qnbefatura/`

### 2. Bağımlılıkların Yüklenmesi (Kritik)
Modülün çalışması için `muhittingulap/qnbfinansbank` kütüphanesinin yüklü olması gerekir. Terminal üzerinden modül klasörüne girerek şu komutu çalıştırın:

```bash
cd modules/addons/qnbefatura/
composer require muhittingulap/qnbfinansbank
```
### 3. Müşteri Özel Alanlarının (Custom Fields) Ayarlanması
...
### 4. Modülün Aktifleştirilmesi ve Ayarlar
